...
const initDb = async () => {
  try {
    ...
    // Агентское вознаграждение убрано из приложения. Ставку удаляем из базы, иначе
    // публичный GET /api/settings начал бы отдавать её всем.
    await pool.query(`DELETE FROM app_settings WHERE key = 'commission_percent';`);
