...
const initDb = async () => {
  try {
    ...
    // Миграции
    await client.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS gold_balance INT DEFAULT 0;'); // <-- ЗОЛОТО
...
// --- ПАРСЕР XML (Тот же, что был, рабочий) ---
...
app.post('/api/auth', async (req, res) => {
  ...
  try {
    ...
    if (dbUser.rows.length === 0) {
      // При регистрации даем 0 золота и 0 серебра
      dbUser = await client.query('INSERT INTO users (telegram_id, username, first_name, gold_balance, balance) VALUES ($1, $2, $3, 0, 0) RETURNING *', [user.id, user.username, user.first_name]);
    }
...
// ... Новости API (оставляем без изменений) ...
...
app.delete('/api/products/:id', async (req, res) => {
  if (await isAdmin(req.body.initData)) {
    // Мы не удаляем, а делаем is_active = false, чтобы сохранить историю
